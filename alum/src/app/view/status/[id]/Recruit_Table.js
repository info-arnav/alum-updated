...
const Recruit_Table = () => {
  ...
  const [branch_filter, set_branch_filter] = useState({
    CSE: false,
    CSAI: false,
    CSDS: false,
    CSDA: false,
  ...
  const branch_filter_handler = (event) => {
    ...
    if (event.target.checked) {
      // set_branch_filter((prev) => {
      //   prev[event.target.value] = true;

      //   return prev;
      // });
      let editBranch = event.target.value;
      set_branch_filter({ ...branch_filter, [editBranch]: true });
    } else {
      // set_branch_filter((prev) => {
      //   prev[event.target.value] = false;

      //   return prev;
      let editBranch = event.target.value;
      set_branch_filter({ ...branch_filter, [editBranch]: false });
    }
  };
    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg">
      <div class="flex items-center justify-between pb-4 bg-white dark:bg-gray-900">
        <div className="m-4 w-1/4">
          <form className="text-xl" onKeyDown={handleKeyDown}>
            <label
              for="steps-range"
            ...
            <div>
              ...
              <div
                id="dropdownSearch"
                class={
                  `z-10 bg-white rounded-lg shadow w-60 dark:bg-gray-00 ` +
                  Branch_Drop
                }
              >
                <ul
            ...
            <button onClick={filter_handler}>Filter</button>
          </form>
        </div>
      </div>
